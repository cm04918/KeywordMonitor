...
class KeywordMonitorPlugin(Plugin):
    ...
    def _handle_violation(self, sender_wxid, group_id, group_name, msg, violation_type):
        ...
        if self.warning_records[sender_wxid] >= self.warning_limit:
            self._kick_user(sender_wxid, group_id, group_name, violation_type, True)
        else:
                ...
                if v_msg_id == recalled_msg_id_str:
                    ...
                    try:
                        self.client.post_text(self.app_id, msg.from_user_id,
                                            ...
                                            ats=user_id)
                        logger.info(f"[KeywordMonitor] 已发送感谢消息给用户 {user_id}")
                    except Exception as e:
                        logger.error(f"[KeywordMonitor] 发送感谢消息失败: {e}")
                    return
    ...
    def _start_countdown(self, sender_wxid, group_id, group_name, violation_msg_id, msg, violation_type):
        ...
        def check_and_notify():
            ...
            with self.recalled_messages_lock:
                if violation_msg_id_str not in self.recalled_messages:
                    logger.info(f"[KeywordMonitor] 用户 {sender_wxid} 未撤回消息[{violation_msg_id_str}]，移出群聊")
                    self._kick_user(sender_wxid, group_id, group_name, violation_type, False)
        ...
        logger.info(f"[KeywordMonitor] 启动倒计时，用户: {sender_wxid}, 消息ID: {violation_msg_id}")

    def _kick_user(self, user_id, group_id, group_name, violation_type, exceed_limit):
        """将用户移出群聊"""
        try:
            self.client.remove_member(self.app_id, group_id, user_id)
            if exceed_limit:
                kick_msg = f"用户 {user_id} 因违规次数达到上限（违规类型: {violation_type}，次数: {self.warning_records[user_id]}），已被移出群聊 {group_name}。"
            else:
                kick_msg = f"用户 {user_id} 因未在规定时间内撤回违规消息（违规类型: {violation_type}，次数: {self.warning_records[user_id]}），已被移出群聊 {group_name}。"
            self.client.post_text(self.app_id, group_id, kick_msg)
            logger.info(f"[KeywordMonitor] 已将用户 {user_id} 移出群聊 {group_name}")
        except Exception as e:
            logger.error(f"[KeywordMonitor] 移出用户 {user_id} 失败: {e}")
